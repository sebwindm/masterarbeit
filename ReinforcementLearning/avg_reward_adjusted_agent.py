...
number_of_evaluation_episodes = 1  # default = 30
...
def rename_results_csv(suffix):
    """
    Renames the csv file generated by the Gym environment and adds the name of the used algorithm/agent
    """
    os.rename("../Evaluation/results.csv","../Evaluation/results_" + str(number_of_machines)
              + "_" + str(suffix) + ".csv")
    return
...
def evaluate_with_default_action(action):
    ...
    print("Final average score over " + str(episodes) + " episodes: " + str(mean(scores)))
    rename_results_csv("default_action_" + str(default_action))
    return scores
...
def evaluate_other_algos(algorithm):
    ...
    print("Final average score over " + str(episodes) + " episodes: " + str(mean(scores)))
    rename_results_csv(str(algorithm) + "_eval")
    return scores
